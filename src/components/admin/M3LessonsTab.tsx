...
export default function M3LessonsTab() {
  ...
  const handleAddM3Lesson = async (e: React.FormEvent) => {
    ...
    formData.append("m3Title", m3Title);
    formData.append("m3Description", m3Description);
    ...
    try {
      const method = editingM3Id ? "PUT" : "POST";
      const res = await fetch("/api/m3-m3Lessons", {
        method,
        body: formData,
      });
      if (res.ok) {
  ...
  const handleEditM3Lesson = (lesson: any) => {
    setM3Title(lesson.m3Title);
    setM3Description(lesson.m3Description);
    setM3Category(lesson.m3Category || "สื่อวิดีทัศน์");
    setM3VideoId(lesson.m3VideoId ? `https://youtube.com/watch?v=${lesson.m3VideoId}` : "");
  ...
  const handleDeleteM3Lesson = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/m3-m3Lessons?id=${id}`, { method: "DELETE" });
      if (res.ok) fetchData();
    } catch (error) {
              ...
              <div className="lg:col-span-1">
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 sticky top-8">
                  <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-4">
                    {editingM3Id ? <Pencil className="text-blue-600" /> : <Plus className="text-blue-600" />}
                    <h2 className="text-lg font-bold text-gray-900">
                      {editingM3Id ? "แก้ไขบทเรียน" : "เพิ่มบทเรียนใหม่"}
                    </h2>
                  </div>
                  <form onSubmit={handleAddM3Lesson} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input type="text" required value={m3Title} onChange={(e) => setM3Title(e.target.value)} className="w-full h-10 px-3 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none text-sm" />
                    </div>
                    <div>
                      <label className="text-sm font-medium text-gray-700 block mb-1">รายละเอียด</label>
                      <textarea required value={m3Description} onChange={(e) => setM3Description(e.target.value)} className="w-full h-20 p-3 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none text-sm resize-none" />
                    </div>
                    ...
                    {m3Category === "สื่อเอกสาร PDF" ? (
                      ...
                      <div>
                        ...
                        <input type="text" required={m3Category !== "สื่อภาพ"} value={m3VideoId} onChange={(e) => setM3VideoId(e.target.value)} className="w-full h-10 px-3 rounded-xl border border-gray-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none text-sm" />
                      </div>
                    ) : (
                      <div>
              ...
              <div className="lg:col-span-2">
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
                  <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-4">
                    ...
                    <h2 className="text-lg font-bold text-gray-900">รายการสื่อการสอน ({m3Lessons.length})</h2>
                  </div>
                  {isLoading ? (
                      ...
                      {m3Lessons.map((lesson) => (
                        <div key={lesson.id} className="flex gap-4 p-3 rounded-2xl border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all group bg-gray-50/50">
                          <div className="w-32 h-20 bg-gray-200 rounded-xl overflow-hidden flex-shrink-0 relative border border-gray-200/50">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img 
                              src={lesson.m3Category === "สื่อภาพ" ? lesson.imageUrl : `https://img.youtube.com/vi/${lesson.m3VideoId}/mqdefault.jpg`} 
                              alt="thumbnail" 
                            ...
                            <div className="absolute top-1 left-1 bg-black/60 backdrop-blur-sm text-white text-[9px] px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">
                              {lesson.m3Category || "สื่อวิดีทัศน์"}
                            </div>
                          </div>
                          <div className="flex-1 min-w-0 py-1">
                            <h3 className="font-bold text-gray-900 truncate text-sm">{lesson.m3Title}</h3>
                            <p className="text-xs text-gray-500 line-clamp-1 mt-1">{lesson.m3Description}</p>
                          ...
                          <div className="flex flex-col justify-center gap-1 flex-shrink-0 px-2">
                            <button onClick={() => handleEditM3Lesson(lesson)} className="w-8 h-8 rounded-lg flex items-center justify-center text-blue-500 hover:bg-blue-100 transition-colors" title="แก้ไข">
                              <Pencil className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleDeleteM3Lesson(lesson.id)} className="w-8 h-8 rounded-lg flex items-center justify-center text-red-500 hover:bg-red-100 transition-colors" title="ลบ">
