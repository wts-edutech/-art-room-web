...
export default function M3MaterialsPage() {
  const [lessons, setLessons] = useState<any[]>([]);
  const [isM3Enabled, setIsM3Enabled] = useState<boolean | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/m3-lessons/status")
      ...
      .catch(() => setIsM3Enabled(true));

    // Fetch M3 lessons
    fetch("/api/m3-lessons")
      .then((res) => res.json())
      .then((data) => {
      ...
      .catch((err) => console.error("Failed to load m3 lessons:", err))
      .finally(() => setIsLoading(false));
  }, []);

  // While checking status
  if (isM3Enabled === null || isLoading) {
    return (
      <>
        <Navbar />
        ...
        <main className="flex-1 flex flex-col items-center justify-center min-h-screen bg-[#FDF9F1] pt-28 pb-16 px-4 text-center">
          <div className="max-w-md w-full bg-white rounded-3xl p-8 sm:p-10 border border-gray-100 shadow-xl shadow-gray-200/50">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">
              สื่อการสอน ม.3 อยู่ระหว่างปรับปรุง
            </h1>
            
            <p className="text-xs sm:text-sm text-gray-500 leading-relaxed mb-6">
              คุณครูผู้สอนกลุ่มสาระการเรียนรู้ศิลปะ กำลังจัดเตรียมบทเรียน สื่อวิดีทัศน์ และใบงานสำหรับระดับชั้น ม.3 เพิ่มเติม กรุณากลับมาติดตามใหม่อีกครั้ง หรือศึกษาในคลังสื่อการสอนทั่วไป
            </p>
    ...
    <>
      ...
      {!isM3Enabled && isAdmin && (
        <div className="bg-amber-500 text-white px-4 py-2.5 text-xs sm:text-sm font-bold flex items-center justify-center gap-2 shadow-md sticky top-16 z-30">
          ...
          <span>โหมดดูตัวอย่าง (Admin Preview): สื่อการสอน ม.3 ยังปิดการแสดงผลหน้าบ้านอยู่ เฉพาะผู้ดูแลระบบเท่านั้นที่มองเห็นหน้านี้ได้</span>
        </div>
      )}
          ...
          <div className="container mx-auto px-4 sm:px-6 max-w-7xl">
            <div className="text-center mb-16">
              <h1 className="text-4xl md:text-5xl font-bold font-heading text-gray-900 mb-6 tracking-tight">
                สื่อการสอน (ระดับชั้น ม.3)
              </h1>
              <p className="text-xl text-gray-600 max-w-2xl mx-auto font-light">
                รวบรวมสื่อการเรียนรู้ เทคนิค และแหล่งค้นคว้าเพิ่มเติมสำหรับนักเรียนระดับชั้น ม.3
              </p>
            </div>
            
            <MaterialsList initialLessons={lessons} basePath="/materials/m3" />
          </div>
        </main>
      </ProtectedRoute>
      <Footer />
