...
export default function M4LessonsTab() {
  ...
  const handleAddM4Lesson = async (e: React.FormEvent) => {
    ...
    formData.append("m4Title", m4Title);
    formData.append("m4Description", m4Description);
    ...
    try {
      const method = editingM4Id ? "PUT" : "POST";
      const res = await fetch("/api/m3-m4Lessons", {
        method,
        body: formData,
      });
      if (res.ok) {
  ...
  const resetM4LessonForm = () => {
    ...
    setM4LessonPdfUrl("");
    const m3PdfInput = document.getElementById("m4LessonPdfInput") as HTMLInputElement;
  ...
  const handleEditM4Lesson = (lesson: any) => {
    setM4Title(lesson.m4Title);
    setM4Description(lesson.m4Description);
    setM4Category(lesson.m4Category || "สื่อวิดีทัศน์");
    setM4VideoId(lesson.m4VideoId ? `https://youtube.com/watch?v=${lesson.m4VideoId}` : "");
  ...
  const handleDeleteM4Lesson = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/m3-m4Lessons?id=${id}`, { method: "DELETE" });
      if (res.ok) fetchData();
    } catch (error) {
                ...
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 sticky top-8">
                  <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-4">
                    ...
                    <h2 className="text-lg font-bold text-gray-900">
                      {editingM4Id ? "แก้ไขบทเรียน" : "เพิ่มบทเรียนใหม่"}
                    </h2>
                  </div>
              ...
              <div className="lg:col-span-2">
                <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
                  <div className="flex items-center gap-2 mb-6 border-b border-gray-100 pb-4">
                    ...
                    <h2 className="text-lg font-bold text-gray-900">รายการสื่อการสอน ({m4Lessons.length})</h2>
                  </div>
                  {isLoading ? (
                      ...
                      {m4Lessons.map((lesson) => (
                        <div key={lesson.id} className="flex gap-4 p-3 rounded-2xl border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all group bg-gray-50/50">
                          <div className="w-32 h-20 bg-gray-200 rounded-xl overflow-hidden flex-shrink-0 relative border border-gray-200/50">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img 
                              src={lesson.m4Category === "สื่อภาพ" ? lesson.imageUrl : `https://img.youtube.com/vi/${lesson.m4VideoId}/mqdefault.jpg`} 
                              alt="thumbnail" 
                            ...
                            <div className="absolute top-1 left-1 bg-black/60 backdrop-blur-sm text-white text-[9px] px-1.5 py-0.5 rounded uppercase font-bold tracking-wider">
                              {lesson.m4Category || "สื่อวิดีทัศน์"}
                            </div>
                          </div>
                          <div className="flex-1 min-w-0 py-1">
                            <h3 className="font-bold text-gray-900 truncate text-sm">{lesson.m4Title}</h3>
                            <p className="text-xs text-gray-500 line-clamp-1 mt-1">{lesson.m4Description}</p>
